...
Every agent registered with MandateZ has a **public agent profile** in the Agent Directory. The profile exposes:
...
This satisfies Article 13's transparency requirement in a machine-readable, cross-vendor format. Deployers inspecting an agent from a different provider can verify its disclosed capabilities against its actual event stream.
...
Yes. Use `npx @mandatez/sdk compliance export --pack eu-ai-act --articles 9,12,13,14`. The output is a PDF plus JSON bundle mapping each article to the specific MandateZ control applied, with the full event trail and signature chain for the reporting window.
...
<script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "MandateZ provides specific controls for Article 9 (risk management via policy rules mapped to OWASP Agentic Top 10), Article 12 (record-keeping via signed agent events), Article 13 (transparency via the public agent profile), and Article 14 (human oversight via the approval gate)."
      }
    },
    {
      "@type": "Question",
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes. Run `npx @mandatez/sdk compliance export --pack eu-ai-act --articles 9,12,13,14`. The output is a PDF plus JSON bundle mapping each article to the specific MandateZ control applied."
      }
    },
    {
      "@type": "Question",
