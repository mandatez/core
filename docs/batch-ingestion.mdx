...
| Events per minute per organization | **10,000** |
| Max request body size | **5 MB** |
| Signature check | Ed25519 per event, rejected batch on any failure |

Requests that exceed the per-request cap return `413 Payload Too Large`.
Partition larger workloads into multiple `trackBatch()` calls.
...
`EventSource` reconnects automatically. For server-side consumers, handle
`onerror` and retry with exponential backoff.
