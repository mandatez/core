...
```bash
npx @mandatez/sdk shadow-scan \
  --surface cloud,saas,service-accounts \
  --window 30d \
...
import { registerDiscoveredAgent } from '@mandatez/sdk';

const registered = await registerDiscoveredAgent({
  shadow_agent_id: 'shadow_7f2a',
  suggested_name: 'Context.ai Indexing Agent',
  suggested_owner: 'context_ai_org',
  initial_policy: 'pol_default_deny',
});
